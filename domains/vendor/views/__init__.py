...
class VendorRegister(APIView):
    ...
    def post(self, request):
        ...
        try:
            result = service.request_registration(serializer.validated_data)
...
class VendorRegisterConfirmation(APIView):
    permission_classes = [AllowAny]

    def post(self, request):
        serializer = VendorRegisterConfirmationSerializer(data=request.data)
        serializer.is_valid(raise_exception=True)
        try:
            result = VendorAuthService().confirm_registration(
                **serializer.validated_data,
            )
        except VendorConfirmationError as exc:
            raise ValidationError(exc.errors) from exc
        return api_response(True, _("Registration confirmed."), result)
