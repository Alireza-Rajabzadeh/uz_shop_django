...
class ProductVariantWriteSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        has_inventory = "inventory" in self.initial_data
        has_serial = "serial_items" in self.initial_data
        if has_inventory and has_serial:
            raise serializers.ValidationError({
                "serial_items": "Submit either inventory or serial_items, not both."
            })
        attrs["inventory_submitted"] = has_inventory or has_serial
        return attrs
