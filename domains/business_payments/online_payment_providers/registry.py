...
from .base import BaseBusinessPaymentProvider


def provider_class(code):
    try:
        module = import_module(
            f"domains.business_payments.online_payment_providers.{code}"
        )
        provider = getattr(module, f"{code.title().replace('_', '')}Provider")
    except (ImportError, AttributeError, ValueError):
        return None
    if not isinstance(provider, type) or not issubclass(
        provider, BaseBusinessPaymentProvider
    ):
        return None
    return provider
